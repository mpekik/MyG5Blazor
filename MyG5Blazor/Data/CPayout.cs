...
using MyG5Blazor.Pages.Configuration;
...
namespace MyG5Blazor.Data
{
    public class CPayout
    {
        ...
        */
        ...
        //Halo DoPoll
        public bool DoPoll(Transaction _trans, int _logBank, int _logAmount)
        {
            byte i;
            ...
            for (i = 1; i < responseLength; ++i)
            {
                switch (response[i])
                {
                    ...
                    case CCommands.SSP_POLL_CREDIT_NOTE:
                        ...
                        UangMasukSekarang = data.Value / 100;
                        _logBank += 1;
                        _logAmount += UangMasukSekarang;
                        config.Write("BILL", Config.PARAM_BILL_AMOUNT, _logAmount.ToString());
                        config.Write("BILL", Config.PARAM_BILL_BANK_STOK, _logBank.ToString());
                        total += UangMasukSekarang;
                        uangCount += 1;
                        OurUtility.Write_Log("Total Uang Yang Diterima : " + total, "step-action");
                        OurUtility.Write_Log("UangMasukSekarang : " + UangMasukSekarang, "step-action");
                        OurUtility.Write_BankLog(_trans.transID + " | " + _trans.jenisTrans + " | " + UangMasukSekarang.ToString(), "NotesLog");
                        UpdateData();
                        i++;
                        return true;
                        break;
        ...
        public bool DoPoll(Transaction _trans, int _logBank, int _logAmount, ref int ctimer)
        {
            byte i;
            Config config = new Config();
